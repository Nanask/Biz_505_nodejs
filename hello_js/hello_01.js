...
// 객체 타입의 변수들을 만들 때 가급적 const를 사용하라
// 함수를 선언할때도 사용
const 상수1 = 300;
 ...
 * 즉, 자바스크립트는 함수형(funcion type) 언어다 라고 한다.
 ...
 * Java로 예를 들면
 * 반드시 클래스, interface 등을 만들고
 * 클래스 내부에서 함수를 선언하여 호출하는 프로그래밍 구조
 * 
 * method는 호출을 하려면
 * 1. 클래스를 객체로 선언, 생성하고  Class class = new Class();
 * 2. 생성된 객체를 통해서 호출을 하는 구조 class.method();
 * 
...
// 함수1 이라는 함수를 선언하는 명령문
const 함수1 = function () {
    console.log("대한민국")
}

// ES6 이전버전(Common JS 버전)에서 사용하던 함수 방식
function func2() {
    console.log("우리나라")
}